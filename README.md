...
[![npm version](https://img.shields.io/npm/v/react-gamepad-hook.svg)](https://www.npmjs.com/package/react-gamepad-hook)
[![npm downloads](https://img.shields.io/npm/dm/react-gamepad-hook.svg)](https://www.npmjs.com/package/react-gamepad-hook)
...
```bash
# npm
npm install react-gamepad-hook

# yarn
yarn add react-gamepad-hook

# pnpm
pnpm add react-gamepad-hook
```
...
import { useGamepad } from 'react-gamepad-hook';
...
function GameControls() {
  ...
  return (
    <div>
      ...
      <p>Movement: X: {moveX.toFixed(2)}, Y: {moveY.toFixed(2)}</p>
...
import { useGamepad, BUTTON_LABELS, AXIS_LABELS } from 'react-gamepad-hook';
    ...
    Object.entries(buttonStates).forEach(([padIndex, buttons]) => {
      Object.entries(buttons).forEach(([buttonIndex, state]) => {
        if (state.pressed) {
          ...
          messages.push(`Gamepad ${padIndex}: ${label} (${state.value.toFixed(2)})`);
        }
      });
    });
...
```jsx
import { BUTTON_LABELS, AXIS_LABELS } from 'react-gamepad-hook';
    ...
    <div className="game-container">
      ...
      {!isGamepadConnected && (
        <div className="warning">
          Please connect a controller to play
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="player">
        <h2>Player 1</h2>
        <p>Movement: X: {player1.moveX.toFixed(2)}, Y: {player1.moveY.toFixed(2)}</p>
        <p>Actions: Jump: {player1.isJumping ? 'Yes' : 'No'}, Fire: {player1.isFiring ? 'Yes' : 'No'}</p>
      </div>
      
      <div className="player">
        <h2>Player 2</h2>
        <p>Movement: X: {player2.moveX.toFixed(2)}, Y: {player2.moveY.toFixed(2)}</p>
        <p>Actions: Jump: {player2.isJumping ? 'Yes' : 'No'}, Fire: {player2.isFiring ? 'Yes' : 'No'}</p>
      </div>
    </div>
  );
}
